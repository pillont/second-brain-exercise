...
from source.controllers.v1.authent.auth_dto import AuthDataDTO
...
def test_to_auth_data_maps_fields() -> None:
    DTO = AuthDataDTO(username="alice", password="secret")

    result = to_auth_data(DTO)
...
def test_to_user_dto_maps_fields() -> None:
    result = to_user_dto(_make_user())

    assert result["id"] == 1
    assert result["username"] == "alice"


def test_to_user_dto_sets_self_link() -> None:
    result = to_user_dto(_make_user())

    assert result["links"]["self_link"]["href"] == "/v1/auth/register"


def test_to_user_dto_sets_login_link() -> None:
    result = to_user_dto(_make_user())
...
def test_to_token_dto_maps_token(app) -> None:
    with app.app_context():
        result = to_token_dto(_make_user())

    assert isinstance(result["token"], str)
    assert len(result["token"]) > 0


def test_to_token_dto_sets_self_link(app) -> None:
    with app.app_context():
        result = to_token_dto(_make_user())
...
def test_to_token_dto_sets_register_link(app) -> None:
    with app.app_context():
        result = to_token_dto(_make_user())
...
def test_to_token_dto_sets_get_all_tasks_link(app) -> None:
    with app.app_context():
        result = to_token_dto(_make_user())
...
def test_to_token_dto_sets_create_task_link(app) -> None:
    with app.app_context():
        result = to_token_dto(_make_user())
