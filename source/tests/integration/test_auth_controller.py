...
def registered_client(client):
    client.post("/v1/auth/register", json=VALID_CREDENTIALS)
    return client


def test_register_returns_201(client) -> None:
    response = client.post("/v1/auth/register", json=VALID_CREDENTIALS)
...
def test_register_returns_self_link(client) -> None:
    ...
    assert data["_links"]["self"]["href"] == "/v1/auth/register"
...
def test_register_returns_login_link(client) -> None:
    ...
    assert data["_links"]["login"]["href"] == "/v1/auth/login"
    assert data["_links"]["login"]["type"] == "POST"


def test_register_missing_username_returns_422(client) -> None:
    response = client.post("/v1/auth/register", json={"password": "testpassword"})

    assert response.status_code == 422


def test_register_missing_password_returns_422(client) -> None:
    response = client.post("/v1/auth/register", json={"username": "testuser"})
...
def test_login_returns_200(registered_client) -> None:
    response = registered_client.post("/v1/auth/login", json=VALID_CREDENTIALS)
...
def test_login_returns_self_link(registered_client) -> None:
    ...
    assert data["_links"]["self"]["href"] == "/v1/auth/login"
...
def test_login_returns_register_link(registered_client) -> None:
    ...
    assert data["_links"]["register"]["href"] == "/v1/auth/register"
...
def test_login_missing_username_returns_422(client) -> None:
    response = client.post("/v1/auth/login", json={"password": "testpassword"})

    assert response.status_code == 422


def test_login_missing_password_returns_422(client) -> None:
    response = client.post("/v1/auth/login", json={"username": "testuser"})
