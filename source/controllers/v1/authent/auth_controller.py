...
v1_auth_blp = Blueprint(
    "auth", __name__, url_prefix="/v1/auth", description="Authentication endpoints."
)


@v1_auth_blp.route("/register", methods=["POST"])
@v1_auth_blp.doc(summary="Register", description="Create a new user account.")
@v1_auth_blp.arguments(AuthDataSchema)
@v1_auth_blp.response(409)
